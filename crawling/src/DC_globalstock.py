...
while True:
    try:
        ...
        BASE_URL = 'https://gall.dcinside.com/mgallery/board/view/?id=tenbagger&no=' + str(start_id)
        response = requests.get(BASE_URL,  headers=headers[0])

        soup = BeautifulSoup(response.content, 'html.parser')
        boards_ID = start_id
        boards_title    = soup.find('title').text.replace(' - 해외주식 마이너 갤러리','')
...
if len(df_ttl) >0:
    ...
    print((datetime.now() + timedelta(hours=9)).strftime('%Y%m%d-%H%M%S') , " DC_GLOBAL-STOCK SUCCESS  : ",len(df_ttl))

else:
    print((datetime.now() + timedelta(hours=9)).strftime('%Y%m%d-%H%M%S') , " DC_GLOBAL-STOCK  NO DATA TO UPDATE")