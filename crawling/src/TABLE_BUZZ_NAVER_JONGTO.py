...
#     print(com)
    ...
    df_already= pd.read_sql("SELECT * FROM TABLE_BUZZ where SOURCE  = '네이버_종목토론실'  and RELATED_ITEM = '{}'".format(df_company.loc[com,'company']),conn)
    df_already
    ...
    while True:
        board_nm +=1
        item_board_url = "https://finance.naver.com/item/board.naver?code=" + df_company.loc[com,'code'] +"&page=" + str(board_nm)
#         print(item_board_url)
        ...
        df_['RELATED_ITEM'] = df_company.loc[com,'company']
    ...
    if len(df_ttl) >0:
        ...
        print((datetime.now() + timedelta(hours=9)).strftime('%Y%m%d-%H%M%S') , "네이버종토_{} SUCCESS  : ".format(df_company.loc[com,'company']),len(df_ttl))
    else:
        print((datetime.now() + timedelta(hours=9)).strftime('%Y%m%d-%H%M%S'), "네이버종토_{} NO DATA TO UPDATE".format(df_company.loc[com,'company']))
