...
df_already= pd.read_sql("SELECT * FROM TABLE_BUZZ where SOURCE  = 'Appstore_m.All' ORDER BY DATETIME desc",conn)
df_already
...
if len(df_ttl) >0:
    ...
    print((datetime.now() + timedelta(hours=9)).strftime('%Y%m%d-%H%M%S')  , " m.All // SUCCESS  : ",len(df_ttl))

else:
    print((datetime.now() + timedelta(hours=9)).strftime('%Y%m%d-%H%M%S') , " m.All //  NO DATA TO UPDATE")
